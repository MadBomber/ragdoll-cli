...
**Important Note**: Ragdoll ONLY supports PostgreSQL in production. The system requires the pgvector extension for vector similarity search and uses PostgreSQL-specific features. SQLite is supported for development and testing only.
...
```ruby
# SQLite is only supported for development and testing
# Production deployments MUST use PostgreSQL with pgvector
config.database_config = {
  adapter: 'sqlite3',
  database: File.join(Dir.home, '.ragdoll', 'ragdoll.sqlite3'),
