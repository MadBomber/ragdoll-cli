  ...
  module Core
    class TextGenerationService
      ...
      def generate_summary(text, max_length: nil)
        ...
        unless @configuration.summarization_config[:enable_document_summarization]
          puts "⚠️  LLM summarization disabled, using fallback (first 500 chars)"
          return text[0..500]
        end

        # Skip if content is too short
        min_length = @configuration.summarization_config[:summary_min_content_length]
        return text if text.length < min_length

        max_length ||= @configuration.summarization_config[:summary_max_length]
        ...
        begin
          if @client == :ruby_llm_configured
            ...
            model = @configuration.summarization_config[:summary_model] || 
                    @configuration.models[:summary] || 
                    @configuration.models[:default]
      ...
      def configure_ruby_llm_if_possible
        ...
        # FIXME: llm_provider is not in current config structure
